...
# Modification of the provided surveySimPP.oy program to perform ONLY selection 
# against the camera footprint (ie implementation of detection failure due to
# gaps between CCDs). Note that this uses the astromentric errors introduced by
# the uncertainty calculations, so all the effects in surveySimPP.py are still 
# calculated, byt detections are not rejected based on brightness. 
